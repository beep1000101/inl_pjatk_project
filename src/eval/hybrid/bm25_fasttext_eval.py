...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate hybrid BM25 -> fastText reranking on POLEVAL2022 passage retrieval dataset."
    )
    parser.add_argument(
        "--dataset-id",
    ...
    if lex_passage_ids.shape != ft_passage_ids.shape or not np.array_equal(lex_passage_ids, ft_passage_ids):
        raise ValueError(
            "Passage id alignment mismatch between BM25 artifacts and fastText vectors. "
