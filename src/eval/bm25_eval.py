...
from src.eval.retrieval_eval import evaluate_and_write_submission, retrieve_bm25_topk
from src.preprocess.bm25_vectors import load_bm25


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate BM25 (Okapi) retrieval on POLEVAL2022 passage retrieval dataset."
    )
    parser.add_argument(
        "--dataset-id",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10_000,
        help="Passage chunk size for vectorized scoring (RAM tradeoff).",
    )
    parser.add_argument(
        "--submission-only",
        action="store_true",
    ...
    bm = cast(dict[str, Any], load_bm25(args.subdataset))
    ...
    append_metrics_csv(
        ...
        row={
            ...
            "bm25_b": float(args.b),
            "bm25_out_dir": str(bm.get("out_dir")) if bm.get("out_dir") is not None else None,
            "bm25_n_passages": meta.get("n_passages"),
            "bm25_n_features": meta.get("n_features"),
            "bm25_min_df": meta.get("min_df"),
            "bm25_max_df": meta.get("max_df"),
            "bm25_max_features": meta.get("max_features"),
            "bm25_avgdl": avgdl,
            "bm25_tokenizer": meta.get("tokenizer"),
        },
    )
